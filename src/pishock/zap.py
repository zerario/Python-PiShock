...
class API:
    ...
    def shocker(
        ...
        """Get an :class:`APIShocker` instance for the given share code.
        ...
        Arguments:
            ...
            name: Used when converting the :class:`APIShocker` to a string,
...
class ShockerInfo(BasicShockerInfo):
    ...
    Used by :meth:`APIShocker.info()`. Calling :meth:`API.get_shockers()` or
    :meth:`SerialShocker.info()` returns a :class:`BasicShockerInfo` instance
    instead.

    This class extends :class:`BasicShockerInfo` with the following attributes:
...
class Shocker:
    ...
    """Base class for :class:`APIShocker` and :class:`SerialShocker`.

    Applications which only need access to :meth:`APIShocker.shock()`,
    :meth:`APIShocker.vibrate()`, :meth:`APIShocker.beep()` and
    :meth:`APIShocker.info()` (with :class:`BasicShockerInfo` only) can swap out
    an :class:`APIShocker` for a :class:`SerialShocker` (with only
